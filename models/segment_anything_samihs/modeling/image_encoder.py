...
class PR_Adapter(nn.Module):
    def __init__(self, adapter_dim, hidden_dim, dropout=0.0):
        super(PR_Adapter, self).__init__()
        self.adapter_rescale = nn.Parameter(torch.empty(1, adapter_dim))
        self.adpater_bias = nn.Parameter(torch.empty(hidden_dim))
        self.dropout = Dropout(dropout)

        nn.init.zeros_(self.adapter_rescale)
        nn.init.zeros_(self.adpater_bias)
    ...
    def forward(self, x, down_projection, up_projection):
        ...
        adapter_output = torch.matmul(adapter_output, up_projection) + self.adpater_bias
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        ### new to ViTEncoder ###
        self.adapter_dim = 100
        self.att_down_projection = nn.Parameter(torch.empty(embed_dim, self.adapter_dim))
        self.mlp_down_projection = nn.Parameter(torch.empty(embed_dim, self.adapter_dim))
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.ratio = 0.25
        self.adapter_dim = 100
        self.attn_adapter = PR_Adapter(adapter_dim=self.adapter_dim, hidden_dim=dim,
                                       dropout=0.0)
        self.mlp_adapter = PR_Adapter(adapter_dim=self.adapter_dim, hidden_dim=dim,
                                       dropout=0.0)
        self.dim = dim
        self.depth = depth

    def forward(self, x: torch.Tensor, attn_projection=None, mlp_projection=None) -> torch.Tensor:
        shortcut = x
        ...
        x = self.norm1(x)
        ### PR Adapters for MHA ###
        if self.attn_adapter is not None and attn_projection is not None:
            x = self.attn_adapter(x, attn_projection, attn_projection.t())
        ### PR Adapters for MHA ###
        x = self.attn(x)
        ...
        if self.mlp_adapter is not None and mlp_projection is not None:
            xn = self.mlp_adapter(xn, mlp_projection, mlp_projection.t())
        ### PR Adapters for MLP ###
        x = x + self.mlp(xn)
